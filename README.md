...
│   │   ├── plan.rs                # config/dojo/plan.toml: timer budgets, SD cases
│   │   ├── problems.rs            # config/dojo/neetcode150.toml: 150 problems, category labels
│   │   ├── state.rs               # ~/.config/netherize/dojo.toml: workspace, language, attempts, spaced redo
...
| Completion popup behavior, acceptance, and auto-trigger after typing | `src/app/event_loop/commands_completion.rs` (`typed_char_completion_action`), `src/app/event_loop/commands_editor.rs` (insert-edit hook), `src/app/event_loop/async_results/` | Per-keystroke trigger policy + debounce live with command helpers; result application lands in async results sub-modules |
| Completion menu look (rows, badges, `(kind) detail` column, footer, width) | `src/render/renderer/editor/completion.rs` (`draw_completion_menu`) | One function renders the menu for BOTH the editor and NetherCanvas cards |
| Completion ordering ("dumb order" after `.`) | `src/async_runtime/scheduler/lsp_parse.rs` (`sortText`), `src/app/app_state/mod.rs` (`from_lsp_items` pre-sort), `src/app/match_ranges.rs` (fuzzy score) | Server `sortText` is the empty-prefix order and the fuzzy-score tiebreak |
...
| Dojo tree, Problem tab, timed sessions, notebook | `src/dojo/`, `src/app/event_loop/commands_dojo.rs`, `config/dojo/`, `src/render/renderer/ui/test_runner.rs` (`build_problem_content`) | Pure rules in `src/dojo`; editor glue (workspace switch, folder/language pickers, debounced statement fetch, clock tick, notebook append) in `commands_dojo.rs`; the left list reuses the Explorer row renderer via `dojo_sidebar_rows`, the right tab is drawn in the test-runner surface |
