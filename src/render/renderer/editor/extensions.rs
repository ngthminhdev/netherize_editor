...
impl Renderer {
    pub fn update_extensions_manager_content(
        ...
        if let Some(cmd_state) = &state.command {
            ...
            let popup_w = ((panel_w * 0.82).max(380.0 * runtime_scale))
                .min(panel_w - 16.0)
                .max(180.0);
            let popup_h = ((panel_h * 0.72).max(260.0 * runtime_scale))
                .min(panel_h - 16.0)
                .max(120.0);
            ...
            chrome.push(
                RegionDrawInstance::new(
                    [popup_x - 1.5, popup_y - 1.5, popup_w + 3.0, popup_h + 3.0],
                    border_color,
                )
                .with_radius(6.0),
            );
            ...
            self.editor_overlay_text_system
                .set_size(Some(log_box_w - 16.0), Some(log_line_h));
        ...
        chrome.push(
            RegionDrawInstance::new(
                [inner_x, shortcuts_y - 2.0, inner_w, shortcut_h + 6.0],
                overlay,
            )
            .with_radius(4.0),
        );
        ...
        for (tab_kind, label) in tabs {
            ...
            chrome.push(
                RegionDrawInstance::new([tab_x, tabs_y - 5.0, tab_w, line_height + 10.0], tab_bg)
                    .with_radius(5.0),
            );
            if active {
                chrome.push(
                    RegionDrawInstance::new(
                        [tab_x, tabs_y + line_height + 3.0, tab_w, 2.0],
                        accent,
                    )
                    .with_radius(1.0),
                );
        ...
        let available_x =
            inner_x + 14.0 + estimate_monospace_width(&installed_label, font_size) + 28.0;
        ...
        if total_virtual_h > content_h {
            ...
            chrome.push(
                RegionDrawInstance::new([track_x, thumb_y, 3.0, thumb_h], with_alpha(ghost, 0.45))
                    .with_radius(1.5),
            );
        }
            ...
            match entry {
                ...
                ExtensionRenderEntry::Item(abs_idx) => {
                    ...
                    if state.expanded_binary.as_deref() == Some(item.binary.as_str()) {
                        ...
                        chrome.push(
                            RegionDrawInstance::new(
                                [inner_x + 12.0, detail_y, inner_w - 24.0, detail_h - 8.0],
                                detail_color,
                            )
                            .with_radius(6.0),
                        );
                        chrome.push(
                            RegionDrawInstance::new(
                                [inner_x + 12.0, detail_y, 3.0, detail_h - 8.0],
                                accent,
                            )
                            .with_radius(1.5),
                        );
