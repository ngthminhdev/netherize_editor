...
pub(super) fn welcome_screen_model(version: &str, recent: &[PathBuf]) -> WelcomeScreenModel {
    WelcomeScreenModel {
        ...
        title: "Netherize".to_string(),
        subtitle: "GPU · Zero Latency · Keyboard Driven".to_string(),
        meta: "Rust · wgpu · by ngthminhdev".to_string(),
        meta_version: format!("v{version}"),
        legend: vec![],
        sections: vec![],
        footer_left: "100% Rust · entire editor rendered on the GPU".to_string(),
        footer_right: "no Electron · no compromise".to_string(),
        quick_actions: vec![
            WelcomeQuickAction::new("New buffer", ":new"),
            WelcomeQuickAction::new("Open project", "⌘ O"),
            WelcomeQuickAction::new("File finder", "spc ff"),
            WelcomeQuickAction::new("Word search", "spc fw"),
        ],
        recent_projects: build_recent_projects(recent),
        sidebar_shortcuts: vec![
            WelcomeShortcut::new(&["⌘", "O"], "Open file or project").important(),
            WelcomeShortcut::new(&["<space>", "p", "j"], "Open recent projects").important(),
            WelcomeShortcut::new(&["<space>", "f", "f"], "File picker (fuzzy find)").important(),
            WelcomeShortcut::new(&["<space>", "f", "w"], "Find word / grep (fzf)").important(),
        ],
        shortcuts_hint: "press ? for all bindings  ·  :help for docs".to_string(),
    }
}
...
mod tests {
    ...
    fn welcome_model_is_compact_empty_state() {
        ...
        assert!(model.quick_actions.iter().any(|a| a.label == "Open project"));
        assert!(model.quick_actions.iter().any(|a| a.label == "File finder"));
        assert!(model.sidebar_shortcuts.iter().any(|s| s.label == "Open file or project"));
        assert!(model.sidebar_shortcuts.iter().any(|s| s.label == "File picker (fuzzy find)"));
