...
pub const DOJO_LANGUAGE: &str = "dojo.language";
pub const DOJO_CHOOSE_FOLDER: &str = "dojo.choose_folder";
...
// Legacy command IDs.
pub const TOGGLE_COLLAPSE_EXPAND: &str = "editor.toggle_collapse_expand";
...
pub const ALL_IDS: &[&str] = &[
    ...
    EXPLORER_EXPAND_COLLAPSE, // legacy
    EXPLORER_OPEN_FILE,
    OVERLAY_SELECT_NEXT,
    OVERLAY_SELECT_PREV,
...
pub fn parse(id: &str, open_file_path: Option<&std::path::Path>) -> Option<Command> {
    match id {
        ...
        DOJO_LANGUAGE => Some(Command::DojoLanguage),
        DOJO_CHOOSE_FOLDER => Some(Command::DojoChooseFolder),
        ...
        TOGGLE_COLLAPSE_EXPAND => Some(Command::ToggleCollapseExpand),
...
mod tests {
    ...
    fn dojo_open_id_round_trips() {
        ...
        assert!(ALL_IDS.contains(&DOJO_LANGUAGE) && ALL_IDS.contains(&DOJO_CHOOSE_FOLDER));
        assert!(matches!(
            parse(DOJO_LANGUAGE, None),
            Some(Command::DojoLanguage)
        ));
        assert!(matches!(
            parse(DOJO_CHOOSE_FOLDER, None),
            Some(Command::DojoChooseFolder)
        ));
    }
